export default async function handler(req, res) {
  ...
  if (req.method === "POST") {
    ...
    const body = { message:"config: update pengaturan", content: Buffer.from(JSON.stringify(newConfig,null,2)).toString("base64") };
    if (sha) body.sha = sha;
    await fetch(`${BASE}/repos/${REPO}/contents/data/config.json`,{method:"PUT",headers:gh,body:JSON.stringify(body)});
    // Update env var cache jika password berubah
