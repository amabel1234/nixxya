...
function NoAuthSignInPage() {
  const [, navigate] = useLocation();
  return (
    <div style={{ display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"100dvh",background:"linear-gradient(145deg,#060414,#0d0726 28%,#160d40 58%,#090f26)",fontFamily:"'Inter',system-ui,sans-serif",padding:"24px" }}>
      <div style={{ maxWidth:400,width:"100%",background:"rgba(255,255,255,.05)",border:"1px solid rgba(124,58,237,.2)",borderRadius:24,padding:"36px 28px",textAlign:"center",backdropFilter:"blur(14px)" }}>
        <div style={{ fontSize:"3rem",marginBottom:12 }}>🧠</div>
        <h2 style={{ color:"#fff",fontSize:"1.5rem",fontWeight:900,margin:"0 0 8px",letterSpacing:"-0.5px" }}>Selamat Datang di Nixx AI</h2>
        <p style={{ color:"rgba(255,255,255,.45)",fontSize:".88rem",marginBottom:28,lineHeight:1.6 }}>26 Model AI gratis untuk kamu. Klik tombol di bawah untuk mulai!</p>
        <button onClick={() => navigate("/chat")} style={{ width:"100%",background:"linear-gradient(135deg,#7c3aed,#a855f7)",color:"#fff",border:"none",borderRadius:14,padding:"14px",fontSize:".95rem",fontWeight:800,cursor:"pointer",marginBottom:10 }}>
          ✨ Langsung Mulai Chat
        </button>
        <button onClick={() => navigate("/")} style={{ width:"100%",background:"transparent",color:"rgba(255,255,255,.45)",border:"1px solid rgba(255,255,255,.1)",borderRadius:14,padding:"11px",fontSize:".85rem",fontWeight:600,cursor:"pointer" }}>
          ← Kembali ke Beranda
        </button>
      </div>
    </div>
    ...
    <QueryClientProvider client={queryClient}>
      <TooltipProvider>
        <Switch>
          ...
          <Route path="/sign-in" component={NoAuthSignInPage} />
          <Route path="/sign-in/*?" component={NoAuthSignInPage} />
          <Route path="/sign-up" component={NoAuthSignInPage} />
          <Route path="/sign-up/*?" component={NoAuthSignInPage} />
...
function App() {
  return (
    <WouterRouter base={basePath}>
      <ThemeProvider>
        {clerkAvailable ? <ClerkProviderWithRoutes /> : <NoAuthRoutes />}
      </ThemeProvider>
    </WouterRouter>
  );
}
