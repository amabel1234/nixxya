import React, { createContext, useContext, useState, useEffect, ReactNode } from "react";
  ...
  export function AuthProvider({ children }: { children: ReactNode }) {
    ...
    const register = async (email: string, username: string, password: string) => {
      ...
      setToken("local-token");
      setUser(authUser);
      // Catat ke server untuk statistik admin
      try {
        const bp = (import.meta.env.BASE_URL ?? "").replace(/\/$/, "");
        await fetch(`${bp}/api/register-user`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ email: newUser.email, username: newUser.username, registeredAt: new Date().toISOString() }),
        });
      } catch (_) { /* tidak memblokir registrasi */ }
    };
