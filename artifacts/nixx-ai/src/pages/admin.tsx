...
export default function AdminPage() {
  ...
  return (
    <div className="nx-admin-page">
      ...
      {error && (
        <div className="nx-admin-error">
          <p>⚠️ {error}</p>
          {user && (
            <div style={{marginTop:12,padding:'10px 14px',background:'rgba(124,58,237,0.1)',borderRadius:8,border:'1px solid rgba(124,58,237,0.2)',fontSize:13}}>
              <p style={{marginBottom:4,fontWeight:600}}>Clerk User ID kamu:</p>
              <code style={{wordBreak:'break-all',color:'#7c3aed'}}>{user.id}</code>
              <p style={{marginTop:6,color:'var(--nx-text-muted)',fontSize:12}}>Kirim ID ini ke admin untuk mendapat akses.</p>
            </div>
          )}
        </div>
      )}
