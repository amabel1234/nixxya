...
/* ── helper: fetch dengan manual timeout ────────────────────────────── */
...
router.post("/openai/chat", async (req, res): Promise<void> => {
  ...
  /* ─── Pollinations non-streaming (paling stabil) ────────────────── */
  try {
  ...
  /* ─── Fallback: Pollinations endpoint langsung (text only) ─────── */
  if (!responseText) {
    try {
